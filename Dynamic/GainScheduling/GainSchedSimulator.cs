...
namespace TimeSeriesAnalysis.Dynamic
{
    ...
    /// </summary>
    /// 
    public class GainSchedSimulator
    {
        GainSchedModel model;
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="model"></param>
        public GainSchedSimulator(GainSchedModel model)
        {
            this.model = model;
        }
        ...
        public void SimulateYmeas(ref GainSchedDataSet processDataSet, double noiseAmplitude=0)
        {
            Simulate(ref processDataSet,true);
            if (noiseAmplitude > 0)
            {
                // use a specific seed here, to avoid potential issues with "random GainSched tests" and not-repeatable
                // errors.
        ...
        /// <param name="writeResultToYmeasInsteadOfYsim">write data to <c>processDataSet.Y_meas</c> instead of <c>processDataSet.Y_sim</c></param>
        ...
        public bool CoSimulate
            ( PidModel pid, ref GainSchedDataSet processDataSet, bool pidActsOnPastStep=true, bool writeResultToYmeasInsteadOfYsim = false)
        {
            ...
            else
            {
                ...
                {
                    processDataSet.Warnings = new List<GainSchedWarnings>();
                }
                processDataSet.Warnings.Add(GainSchedWarnings.FailedToInitializeGainSchedModel);
                Debug.WriteLine("Failed to initalize gain scheduling model.");
                u = umin + (umax-umin)/2;
            }
        ...
        public double[] Simulate(ref GainSchedDataSet processDataSet,
