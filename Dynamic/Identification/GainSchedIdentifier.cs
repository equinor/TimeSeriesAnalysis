...
namespace TimeSeriesAnalysis.Dynamic
{
    ...
    /// which are a different parametrization. The GainSchedIdentifier, GainSchedModel and GainSchedParamters classes handle this transition seamlessly to the user.
    ...
    /// </para>
    /// <para>
    /// Another advantage of the paramterization, is that the model internally separates between stedy-state and transient state, you can at any instance
    ...
    /// is deferred to <seealso cref="GainSchedTimeDelayIdentifier"/>
    /// </para>
    /// <para>
    ...
    public class GainSchedIdentifier
    {
        ...
        const bool doGainSchedyUNorm = true;// if set true, then Unorm is always one
        /// <summary>
        /// Default Constructor
        /// </summary>
        public GainSchedIdentifier()
        {
        }
        ...
        public GainSchedModel Identify(ref GainSchedDataSet dataSet,
        ...
        /// <returns> the identified model parameters and some information about the fit</returns>
        public GainSchedModel IdentifyStatic(ref GainSchedDataSet dataSet, FittingSpecs fittingSpecs = null)
        {
        ...
        /// <returns> the identified model parameters and some information about the fit</returns>
        public GainSchedModel IdentifyLinear(ref GainSchedDataSet dataSet, FittingSpecs fittingSpecs, bool doEstimateTimeDelay = true)
        {
        ...
        /// <param name="uNorm">normalizing paramter for u-u0 (its range)</param>
        /// <returns> the identified model parameters and some information about the fit</returns>
        public GainSchedModel IdentifyLinearDiff(ref GainSchedDataSet dataSet, FittingSpecs fittingSpecs, bool doEstimateTimeDelay = true)
        {
            var diffDataSet = new GainSchedDataSet(dataSet);
            ...
            {
                var simulator = new GainSchedSimulator(model);
        ...
        /// <param name="uNorm">normalizing paramter for u-u0 (its range)</param>
        /// <returns> the identified model parameters and some information about the fit</returns>
        public GainSchedModel IdentifyLinearAndStatic(ref GainSchedDataSet dataSet, FittingSpecs fittingSpecs, bool doEstimateTimeDelay = true)
        {
        ...
        /// <param name="uNorm">normalizing paramter for u-u0 (its range)</param>
        /// <returns> the identified model parameters and some information about the fit</returns>
        public GainSchedModel IdentifyLinearAndStaticDiff(ref GainSchedDataSet dataSet, FittingSpecs fittingSpecs, bool doEstimateTimeDelay = true)
        {
            ConvertDatasetToDiffForm(ref dataSet);
            return Identify_Internal(ref dataSet, fittingSpecs, false, false, doEstimateTimeDelay);
        }

        private void ConvertDatasetToDiffForm(ref GainSchedDataSet dataSet)
        {
        ...
        /// <returns> the identified model parameters and some information about the fit</returns>
        private GainSchedModel Identify_Internal(ref GainSchedDataSet dataSet, FittingSpecs fittingSpecs,
            bool doUseDynamicModel = true, bool doEstimateCurvature = true, bool doEstimateTimeDelay = true)
        {
            ...
            GainSchedTimeDelayIdentifier processTimeDelayIdentifyObj =
                new GainSchedTimeDelayIdentifier(dataSet.GetTimeBase(), maxExpectedTc_s);
            ...
            // 
            var modelList = new List<GainSchedParameters>();
            int timeDelayIdx = 0;
            GainSchedParameters modelParams_StaticAndNoCurvature =
            ...
            timeDelayIdx = 0;
            GainSchedParameters modelParams = null;
            while (continueIncreasingTimeDelayEst)
            {
                modelParams = null;
                GainSchedParameters modelParams_noCurvature =
                    EstimateProcessForAGivenTimeDelay
                ...
                {
                    GainSchedParameters modelParams_allCurvature =
                         EstimateProcessForAGivenTimeDelay
                        ...
                        modelParams_allCurvature.Fitting.RsqDiff - modelParams_noCurvature.Fitting.RsqDiff > rSquaredDiff_MinImprovement)
                    {
                        ...
                        {
                            GainSchedParameters modelParams_withCurvature =
                                    EstimateProcessForAGivenTimeDelay
                ...
                /*   bool doDebugPlotting = false;//should normally be false.
                   if (doDebugPlotting)
                   { 
                       var debugModel = new GainSchedModel(modelParams, dataSet);
                       var sim = new GainSchedSimulator(debugModel);
            ...
            GainSchedParameters modelParameters =
                (GainSchedParameters)processTimeDelayIdentifyObj.GetRun(bestTimeDelayIdx);
            ...
            {
                modelParameters.AddWarning(GainScheddentWarnings.ConstantInputU);
            }
            if (correlatedInputInds.Count > 0)
            {
                modelParameters.AddWarning(GainScheddentWarnings.CorrelatedInputsU);
            }
            ...
            // END While loop 
            /////////////////////////////////////////////////////////////////
            var model = new GainSchedModel(modelParameters, dataSet);
        ...
        private GainSchedParameters ChooseBestModel(GainSchedParameters fallbackModel, List<GainSchedParameters> allModels)
        {
            GainSchedParameters bestModel = fallbackModel;
            ...
            foreach (GainSchedParameters curModel in allModels)
            {
        ...
        private GainSchedParameters EstimateProcessForAGivenTimeDelay
            (int timeDelay_samples, GainSchedDataSet dataSet,
            ...
            )
        {
            ...
            GainSchedParameters parameters = new GainSchedParameters();
            ...
            if (regResults.Param == null || !regResults.AbleToIdentify)
            {
                ...
                parameters.AddWarning(GainScheddentWarnings.RegressionProblemFailedToYieldSolution);
                return parameters;
            }
            else if (regResults.Param.Contains(Double.NaN) || linearProcessGains.Contains(Double.NaN))
            {
                parameters.Fitting.WasAbleToIdentify = false;
                parameters.AddWarning(GainScheddentWarnings.RegressionProblemNaNSolution);
                return parameters;
            }
            else if (Math.Abs(regResults.Param[1]) > maxAbsValueRegression)
            {
                parameters.Fitting.WasAbleToIdentify = false;
                parameters.AddWarning(GainScheddentWarnings.NotPossibleToIdentify);
                return parameters;
            }
            else // able to identify
            {
                ...
                parameters.TimeConstant_s = null; // TODO: SignificantDigits.Format(timeConstant_s, nDigits);
                parameters.LinearGains = null; // TODO: SignificantDigits.Format(linearProcessGains, nDigits);
                parameters.U0 = u0;
                ...
                {
                    parameters.AddWarning(GainScheddentWarnings.ReEstimateBiasFailed);
                ...
                // parameters.Fitting.CalcCommonFitMetricsFromDataset(dataSet, yIndicesToIgnore); // TODO: fitting for GainSched
                ...
                //     parameters.TimeConstantUnc_s = SignificantDigits.Format(parameters.TimeConstantUnc_s.Value, nDigits); // TODO: uncertainty in time constants
                return parameters;
            }
        }
        ...
        private void CalculateStaticUncertainty(RegressionResults regResults, ref GainSchedParameters parameters)
        {
        ...
        /// <param name="timeBase_s"></param>
        /// <param name="parameters"></param>
        private void CalculateDynamicUncertainty(RegressionResults regResults, double timeBase_s, ref GainSchedParameters parameters)
        {
            if (regResults.VarCovarMatrix == null)
                return;
            ...
            // parameters.TimeConstantUnc_s = standardErrorTc *1.96;//95 prc unceratinty // TODO: make this for array type too
        ...
        //
        static public (double?, double[]) SimulateAndReEstimateBias(GainSchedDataSet dataSet, GainSchedParameters parameters)
        {
            GainSchedDataSet internalData = new GainSchedDataSet(dataSet);
            parameters.Bias = 0;
            double nanValue = internalData.BadDataID;
            var model = new GainSchedModel(parameters);
            var simulator = new GainSchedSimulator(model);
        ...
        public GainSchedModel IdentifyLinearAndStaticWhileKeepingLinearGainFixed(GainSchedDataSet dataSet, int inputIdxToFix,
            double inputProcessGainValueToFix, double u0_fixedInput, double uNorm_fixedInput)
        {
            var internalDataset = new GainSchedDataSet(dataSet);
            var vec = new Vec(dataSet.BadDataID);
            ...
            var idGainSchedModel = IdentifyLinearAndStatic(ref internalDataset, new FittingSpecs());
            if (!idGainSchedModel.modelParameters.Fitting.WasAbleToIdentify)
                return idGainSchedModel;
            //trick now is to add back the paramters that are fixed to the returned model:
            var idLinGains = idGainSchedModel.modelParameters.LinearGains;
            var idU = idGainSchedModel.modelParameters.U0;
            var idUnorm = idGainSchedModel.modelParameters.UNorm;
            ...
            var curIdInput = 0;
            // for (int curInputIdx = 0; curInputIdx < idGainSchedModel.modelParameters.LinearGains.Length + 1; curInputIdx++) // TODO: .Length does not work because of array
            // {
            ...
            //     else
            //     {
            //         // newLinGainsList.Add(idLinGains.ElementAt(curIdInput)); // TODO: removed this
            ...
            var newParams = new GainSchedParameters();
            // newParams.LinearGains = newLinGainsList.ToArray(); // TODO: removed this
            ...
            for (int idx = 0; idx < newLinGainsList.Count; idx++)
            {
                ...
                {
                    newParams.LinearGainUnc[idx] = idGainSchedModel.modelParameters.LinearGainUnc[counter];
                    counter++;
                }
            ...
            newParams.Bias = idGainSchedModel.modelParameters.Bias;
            newParams.Fitting = idGainSchedModel.modelParameters.Fitting;
            newParams.Fitting.SolverID = "Linear,static while fixing index:" + inputIdxToFix;
            var retGainSchedModel = new GainSchedModel(newParams);
            retGainSchedModel.SetFittedDataSet(idGainSchedModel.GetFittedDataSet());
            return retGainSchedModel;
        }
    }
}
